...
cc._RF.push(module, 'de8bcbijfdOYYrMqhHWFdPa', 'game', __filename);
// Script/game.js
...
cc.Class({
    ...
    //显示赔率
    showOdds: function showOdds(res, header) {
        if (res) {
    ...
    setkjInfo: function setkjInfo(res, header) {
        if (res.preNum) {
    ...
    showTrend: function showTrend(res, header) {
        if (res.length > 0) {
    ...
    //检测是否已经开奖结算
    checkDrawed: function checkDrawed() {
        this.schedule(this.kjfun, 2);
    },
    kjfun: function kjfun() {
    ...
    settleAccounts: function settleAccounts(res, header) {
        if (res) {
    ...
    //格式化分秒
    fftime: function fftime(a) {
        return Number(a) < 10 ? "" + 0 + Number(a) : Number(a);
    },
    //禁用投注按钮和机器人投币
    banBetbtn: function banBetbtn() {
        this.unschedule(this.creatOthergold);
        if (this.longBetbtn.interactable) this.longBetbtn.interactable = false;
        if (this.huBetbtn.interactable) this.huBetbtn.interactable = false;
        if (this.heBetbtn.interactable) this.heBetbtn.interactable = false;
        if (this.confirmBetbtn.interactable) this.confirmBetbtn.interactable = false;
        if (this.resetBtn.interactable) this.resetBtn.interactable = false;
    },
    //激活投注按钮
    activateBetbtn: function activateBetbtn() {

        if (!this.longBetbtn.interactable) this.longBetbtn.interactable = true;
        if (!this.huBetbtn.interactable) this.huBetbtn.interactable = true;
        if (!this.heBetbtn.interactable) this.heBetbtn.interactable = true;
        if (!this.confirmBetbtn.interactable) this.confirmBetbtn.interactable = true;
    ...
    //显示到前端封盘倒计时时间格式
    getBetTimeStr: function getBetTimeStr(a) {
        if (!a) {
            this.banBetbtn();
            Global.fengpan = true;
            return "已封盘";
    ...
    //显示到前端开奖倒计时时间格式
    getDrawTimeStr: function getDrawTimeStr(b) {
        if (!b) {
            this.banBetbtn();
            return "已结束";
        }
    ...
    //创建系统的金币投放效果
    creatOthergold: function creatOthergold() {
    ...
    //清空桌面金币
    clearGoldsprite: function clearGoldsprite() {
        //用户自己的下注金币
        ...
        for (var i = 0; i < deskchild.length; i++) {
            ...
            if (goldboxchild.length > 0) {
                for (var idx = 0; idx < goldboxchild.length; idx++) {
